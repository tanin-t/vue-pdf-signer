...
import { setupCanvasForDesktop } from './desktop'
...
function renderIcon (icon: HTMLImageElement) {
  return function renderIcon (ctx: CanvasRenderingContext2D, left: number, top: number, fabricObject: fabric.Object) {
    ...
    ctx.drawImage(icon as any, -size / 2, -size / 2, size, size)
    ctx.restore()
  }
}
...
// async function drawPDFPages (canvas: fabric.Canvas, wrapper: HTMLElement, pdfUrl: string) {
...
//   const pdfImages = await renderPDF(pdfUrl)
...
export function setupCanvas (canvasId: string, pdfUrl: string) {
  setupObjectControls()
  if (isMobile()) {
    const controller = new MobileCanvasController()
    controller.setup(canvasId, pdfUrl)
    return controller
  } else {
    const controller = new MobileCanvasController()
    controller.setup(canvasId, pdfUrl)
